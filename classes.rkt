...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;;;;;;;;;;;;;CLASSES AND DETAILING OF PROPERTIES AND PLAYER;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
;;basic property-elements and there initialisations
...
(define game-player%
  (class object%
    ...
    (define/public (wealth)
      ...
      (+ x balance))
    (define/public (not-bankrupt)
      (if  bankruptcy (void)
      (if (> (wealth) 0) (set! bankruptcy #f) (set! bankruptcy #t))))
    (define/public (purchase property) ;;property is an object of class standard property
      (if (<= balance (get-field purchase-price property)) (displayln "NOT ENOUGH BALANCE TO BUY")
       (begin
    ...
    (define/public (upgrade-property property)
      (if (equal? (hash-ref color-sets (get-field color property)) (frequency-str (get-field color property) (color-giver))) 
      (if (<= balance (get-field build-house property)) (displayln "NOT ENOUGH BALANCE TO UPGRADE")
    ...
    (define/public (get-out-of-jail)
      (begin (set! balance (- balance 50))
             ...
             (displayln balance)
             (newline)
             (displayln "Position is restored")
             (newline)))
    ...
    (define/public (supertax)
      ...
      (displayln balance)
      (newline)))
    (define/public (round-completion)
      (if (> distance (modulo distance 40)) (begin (set! balance (+ balance 200))
                                      (displayln "You recieved $200 for completing one round")
                                      (display "Player : ")
                                      (displayln playername)
                                      (display "Balance : ")
                                      (displayln balance)
                                      (newline)
                                      (set! distance (modulo distance 40)))
          (void 1)))
    (define/public (community-chest)
      (define a (random 0 2))
      (if (= a 0) (begin (set! balance (- balance 100))
                          (displayln "Oops! Bad Luck! $100 have been deducted")
                                      (display "Player : ")
                                      (displayln playername)
                                      (display "Balance : ")
                                      (displayln balance)
                                      (newline))
          (begin (set! balance (+ balance 100))
                          (displayln "Congratulations ! You earned $100")
                                      (display "Player : ")
                                      (displayln playername)
                                      (display "Balance : ")
                                      (displayln balance)
                                      (newline))))
    (define/public (property-name-list)
     (map (lambda (x) (get-field property-name (hash-ref boxes (hash-ref tagged-hash x)))) 
      list-of-properties))
    ))
  ...
  (init-field [build-house 0])
   ...
   (define/public (upgrade)
     (cond [(= upgrade-count 0) (begin (set! rent rent0) (set! upgrade-count (+ 1 upgrade-count)))]
           ...
           [else (displayln "CANNOT BE UPGRADED")]))
...
(set-field! property-name (hash-ref boxes "box14") "NORTHUMR-'D-AVENUE")
...
(set-field! image (hash-ref boxes "box25") (bitmap "images/king_s cross station.jpg"))
